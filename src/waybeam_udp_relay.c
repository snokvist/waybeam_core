...
"header{display:flex;gap:.75rem;flex-wrap:wrap;align-items:flex-end;padding:12px 16px;background:#0c0f17;position:sticky;top:0;z-index:2;border-bottom:1px solid #1e2231}"
"h1{font-size:16px;margin:0 12px 0 0;letter-spacing:.2px}.row{display:flex;gap:.5rem;align-items:center}"
...
"main{max-width:1100px;margin:16px auto;padding:0 16px;display:grid;grid-template-columns:1.2fr .8fr;gap:16px}"
...
".dest{min-height:80px;border:2px dashed #2b344d;border-radius:14px;display:flex;align-items:center;justify-content:center;gap:10px;padding:12px;transition:.15s}"
".dest.drag-over{border-color:var(--accent);background:#0f1a14}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}"
".muted{color:var(--muted)}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,\"Liberation Mono\",monospace}"
".list{display:flex;flex-direction:column;gap:8px}.rowline{display:flex;justify-content:space-between;gap:6px;border:1px solid #252d40;border-radius:10px;padding:8px 10px}"
...
"<div class='row'><label>Receiver host</label><input id='rxHost' value='127.0.0.1' style='width:140px'/>"
"<label>port</label><input id='rxPort' value='5600' style='width:100px'/>"
"<button class='primary' id='refresh'>Refresh</button></div>"
"</header>"
"<main>"
"<section><h2>Drag any bind → Receiver</h2><div class='pad'>"
"<div class='chips' id='bindChips'></div>"
"<div id='dropZone' class='dest' style='margin-top:12px'><div>"
"<div class='muted'>Drop a bind here to route it to</div>"
"<div class='mono' id='rxLabel'>127.0.0.1:5600</div>"
"</div></div><div style='margin-top:10px' class='muted'>Drop action: clears ALL bind destinations, then sets the dropped bind to the receiver.</div>"
"</div></section>"
"<section><h2>Live Status</h2><div class='pad' id='statusBox'><div class='muted'>Loading…</div></div></section>"
"<section><h2>Binds (add/remove)</h2><div class='pad'><div class='grid'>"
"<div class='list' id='bindList'></div>"
"<div><div class='row' style='gap:8px;margin-bottom:8px'>"
"<input id='newBindPort' placeholder='e.g. 5700' style='width:140px'/>"
"<button id='addBind'>Add bind</button></div>"
"<button class='warn' id='applyCfg'>Apply config</button>"
"<div style='height:8px'></div><label>Config (INI)</label>"
...
"(function(){const $=s=>document.querySelector(s);const log=(m,c='')=>{const el=$('#log');const d=document.createElement('div');if(c)d.className=c;d.textContent=`[${new Date().toLocaleTimeString()}] ${m}`;el.prepend(d)};"
"const state={status:null,ini:''};const GET=p=>fetch(p,{method:'GET'}).then(r=>{if(!r.ok)throw new Error(`${r.status} ${r.statusText}`);return p.includes('/status')?r.json():r.text()});"
...
"const rxSync=()=>{$('#rxLabel').textContent=`${$('#rxHost').value.trim()}:${$('#rxPort').value.trim()}`};"
"$('#rxHost').addEventListener('input',rxSync);$('#rxPort').addEventListener('input',rxSync);"
"function makeChip(port,stats){const el=document.createElement('div');el.className='chip';el.draggable=true;el.dataset.port=String(port);el.innerHTML=`<strong>bind ${port}</strong> <span class='tag'>in: ${stats.pkts_in||0}</span>`;el.addEventListener('dragstart',e=>{e.dataTransfer.setData('text/plain',String(port));e.dataTransfer.effectAllowed='copy'});return el}"
"const drop=$('#dropZone');drop.addEventListener('dragover',e=>{e.preventDefault();drop.classList.add('drag-over');e.dataTransfer.dropEffect='copy'});drop.addEventListener('dragleave',()=>drop.classList.remove('drag-over'));"
"drop.addEventListener('drop',async e=>{e.preventDefault();drop.classList.remove('drag-over');const srcPort=parseInt(e.dataTransfer.getData('text/plain'),10);if(!srcPort)return;const host=$('#rxHost').value.trim()||'127.0.0.1';const dport=parseInt($('#rxPort').value.trim(),10)||5600;try{log(`Routing bind ${srcPort} → ${host}:${dport} (clearing others)…`);const rels=(state.status&&state.status.relays)?state.status.relays:[];for(const r of rels){await action('clear',{port:r.port})}await action('set',{port:srcPort,dests:[`${host}:${dport}`]});log(`OK: ${srcPort} now → ${host}:${dport}`,'ok');await loadStatus()}catch(err){log(`Drop failed: ${err}`,'err')}});"
"function renderBindListFromIni(){const lines=state.ini.split(/\\n/);const binds=[];for(const ln of lines){const m=ln.match(/^\\s*bind\\s*=\\s*(\\d+)/);if(m)binds.push(parseInt(m[1],10))}const list=$('#bindList');list.innerHTML='';if(!binds.length){list.innerHTML='<div class=\"muted\">No binds in config.</div>';return}for(const p of binds){const row=document.createElement('div');row.className='rowline';row.innerHTML=`<div><strong class='mono'>bind=${p}</strong></div>`;const btns=document.createElement('div');const rm=document.createElement('button');rm.textContent='Remove';rm.onclick=()=>removeBindInIni(p);btns.appendChild(rm);row.appendChild(btns);list.appendChild(row)}}"
...
"function addBindInIni(port){const p=parseInt(port,10);if(!(p>0&&p<65536)){log('Invalid port','err');return}if(new RegExp(`^\\\\s*bind\\\\s*=\\\\s*${p}(\\\\b|:)`,'m').test(state.ini)){log(`bind ${p} already exists`,'warn');return}if(!/\\n$/.test(state.ini))state.ini+='\\n';state.ini+=`bind=${p}\\n`;$('#cfgText').value=state.ini;renderBindListFromIni()}"
"$('#addBind').onclick=()=>addBindInIni($('#newBindPort').value.trim());$('#applyCfg').onclick=async()=>{try{await POST('/api/v1/config',$('#cfgText').value);log('Config applied & reloaded','ok');await refreshAll()}catch(err){log(`Apply failed: ${err}`,'err')}};$('#refresh').onclick=()=>refreshAll();"
"async function loadStatus(){try{const s=await GET('/api/v1/status');state.status=s;renderStatus(s);renderBindChips(s)}catch(err){$('#statusBox').innerHTML=`<span class='err'>Status error:</span> ${err}`}}"
"async function loadConfig(){try{const t=await GET('/api/v1/config');state.ini=String(t||'').replace(/\\r/g,'');$('#cfgText').value=state.ini;renderBindListFromIni()}catch(err){state.ini='';$('#cfgText').value='';$('#bindList').innerHTML=`<span class='err'>Config error:</span> ${err}`}}"
"function renderStatus(s){const box=$('#statusBox');if(!s||!Array.isArray(s.relays)){box.innerHTML='<div class=\"muted\">No relays</div>';return}const frag=document.createDocumentFragment();for(const r of s.relays){const line=document.createElement('div');line.className='rowline';const left=document.createElement('div');left.innerHTML=`<strong>bind ${r.port}</strong><div class='muted'>pkts_in ${r.pkts_in} · bytes_out ${r.bytes_out}</div>`;const right=document.createElement('div');const ip=document.createElement('input');ip.placeholder='ip or blank=127.0.0.1';ip.style.width='160px';const port=document.createElement('input');port.placeholder='port';port.style.width='90px';const set=document.createElement('button');set.textContent='Set dest';set.onclick=async()=>{const d=port.value.trim();if(!d){log('Port required','warn');return}const host=ip.value.trim();const token=host?`${host}:${d}`:d;try{await action('set',{port:r.port,dests:[token]});log(`bind ${r.port} → ${token}`,'ok');await loadStatus()}catch(err){log(`set failed: ${err}`,'err')}};const clr=document.createElement('button');clr.textContent='Clear';clr.onclick=async()=>{try{await action('clear',{port:r.port});log(`cleared ${r.port}`,'ok');await loadStatus()}catch(err){log(`clear failed: ${err}`,'err')}};right.appendChild(ip);right.appendChild(port);right.appendChild(set);right.appendChild(clr);const dests=document.createElement('div');dests.className='chips';dests.style.marginTop='6px';if(Array.isArray(r.dests)&&r.dests.length){for(const d of r.dests){const c=document.createElement('div');c.className='chip';c.innerHTML=`<span class='tag'>→</span><strong>${d.ip}:${d.port}</strong> <span class='tag'>pkts ${d.pkts}</span>`;dests.appendChild(c)}}else{const m=document.createElement('div');m.className='muted';m.textContent='no destinations';dests.appendChild(m)}line.appendChild(left);line.appendChild(right);frag.appendChild(line);frag.appendChild(dests)}box.innerHTML='';box.appendChild(frag)}"
"function renderBindChips(s){const wrap=$('#bindChips');wrap.innerHTML='';const rel=(s&&s.relays)?s.relays:[];for(const r of rel){wrap.appendChild(makeChip(r.port,r))}}"
"async function refreshAll(){rxSync();await Promise.all([loadStatus(),loadConfig()])}"
